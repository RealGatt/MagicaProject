...
public enum ParticleEffect {
	/**
	 * Umbaskas version of enum names
	 */
	...
	ITEM_BREAK("item break", 36, -1, ParticleProperty.DIRECTIONAL, ParticleProperty.REQUIRES_DATA),
	
	TILE_BREAK("tile break", 37, -1, ParticleProperty.DIRECTIONAL, ParticleProperty.REQUIRES_DATA),
	BLOCK_BREAK("block break", 37, -1, ParticleProperty.DIRECTIONAL, ParticleProperty.REQUIRES_DATA),
	TILE_DUST("tile dust", 38, 7, ParticleProperty.DIRECTIONAL, ParticleProperty.REQUIRES_DATA),
	BLOCK_DUST("block dust", 38, 7, ParticleProperty.DIRECTIONAL, ParticleProperty.REQUIRES_DATA),
	
	WATER_DROP("waterdrop", 39, 8),
	ITEM_TAKE("itemTake", 40, 8),
	MOB_APPEARANCE("mobAppearance", 41, 8),
	DRAGON_BREATH("dragonBreath", 42, 9, ParticleProperty.DIRECTIONAL),										//1.9  Particles
	END_ROD("endRod", 43, 9, ParticleProperty.DIRECTIONAL),													//1.9  Particles
	DAMAGE_INDICATOR("damageIndicator", 44, 9, ParticleProperty.DIRECTIONAL),								//1.9  Particles
	SWEEP_ATTACK("sweepAttack", 45, 9),										//1.9  Particles
	FALLING_DUST("fallingdust", 46, 10, ParticleProperty.REQUIRES_DATA),		//1.10 Particles
	TOTEM("totem", 47, 11, ParticleProperty.DIRECTIONAL), 													//1.11 Particles
	SPIT("spit", 48, 11, ParticleProperty.DIRECTIONAL)	;													//1.11 Particles

	public static final Map<String, ParticleEffect> NAME_MAP = new HashMap<String, ParticleEffect>();
	...
	private static boolean isDataCorrect(ParticleEffect effect, ParticleData data) {
		return ((effect == BLOCK_BREAK || effect == BLOCK_DUST || effect == FALLING_DUST) && data instanceof BlockData) || ((effect == ITEM_BREAK) && data instanceof ItemData);
	}
	...
	/**
	 * Represents the particle data for effects like {@link ParticleEffect#ITEM_BREAK}, {@link ParticleEffect#BLOCK_BREAK} and {@link ParticleEffect#BLOCK_DUST}
	 * <p>
	 * This class is part of the <b>ParticleEffect Library</b> and follows the same usage conditions
	 * 
	 * @author DarkBlade12
	...
	/**
	 * Represents the item data for the {@link ParticleEffect#ITEM_BREAK} effect
	 * <p>
	 * This class is part of the <b>ParticleEffect Library</b> and follows the same usage conditions
	 * 
	 * @author DarkBlade12
	...
	/**
	 * Represents the block data for the {@link ParticleEffect#BLOCK_BREAK} and {@link ParticleEffect#BLOCK_DUST} effects
	 * <p>
	 * This class is part of the <b>ParticleEffect Library</b> and follows the same usage conditions
	 * 
	 * @author DarkBlade12
	...
	/**
	 * Represents the color for effects like {@link ParticleEffect#MOB_SPELL}, {@link ParticleEffect#MOB_SPELL_AMBIENT}, {@link ParticleEffect#RED_DUST} and {@link ParticleEffect#NOTE}
	 * <p>
	 * This class is part of the <b>ParticleEffect Library</b> and follows the same usage conditions
	 * 
	 * @author DarkBlade12
	...
	/**
	 * Represents the color for effects like SPELL, SPELL_MOB and NOTE
	 * <p>
	 * This class is part of the <b>ParticleEffect Library</b> and follows the same usage conditions
	 * 
	 * @author DarkBlade12
	...
	public static final class ParticlePacket {
		...
		public static void initialize() throws VersionIncompatibleException {
			...
			try {
				//Version.TODO make this work for 10.0+ :3
				version = Integer.parseInt(Character.toString(PackageType.getServerVersion().charAt(3)));
				if (version == 1 && Integer.parseInt(Character.toString(PackageType.getServerVersion().charAt(4))) >= 0){
					version = Integer.parseInt(Integer.parseInt(Character.toString(PackageType.getServerVersion().charAt(3))) +""+ Integer.parseInt(Character.toString(PackageType.getServerVersion().charAt(4))));
				}
				if (version > 7) {
			...
			try {
				...
				if (version < 8) {
					...
					if (data != null) {
						...
						ReflectionUtils.setValue(packet, true, "k", (effect == ParticleEffect.ITEM_BREAK) ? packetData : new int[] { packetData[0] | (packetData[1] << 12) });
					}
				}
     ...
     * 
     * 
     * @param particleCount 	How many particles should spawn
     * @param dataMat 			The material ex. Material.Stone		//TODO possibly use ParticleData instead of this and make method for using it
     * @param dataID			The byte id of the material
     * @param speed				Display speed of the particles
     * @param offsetX			Maximum distance particles can fly away from the center on the x-axis
     * @param offsetY			Maximum distance particles can fly away from the center on the y-axis
     * @param offsetZ			Maximum distance particles can fly away from the center on the z-axis
     * @param visibleRange		Range of the visibility
	 * 
	*/
	public void display(Integer particleCount, Material dataMat, byte dataID, Number speed, Number offsetX, Number offsetY, Number offsetZ, Location[] locations, double visibleRange, List<Player> players) {
		if (!this.isSupported()) {
			return;
        }
		if (this == ParticleEffect.RED_DUST || this == ParticleEffect.MOB_SPELL || this == ParticleEffect.MOB_SPELL_AMBIENT) {
			Integer r = offsetX.intValue();
			Integer g = offsetY.intValue();
			Integer b = offsetZ.intValue();
			for (Location center : locations) {
				if (players != null){
					for (Player p : players) {
						display(new OrdinaryColor(r, g, b), center, p);
					}
				} else {
					display(new OrdinaryColor(r, g, b), center, visibleRange);
				}
			}
		}else if (this == ParticleEffect.NOTE){
			for (Location center : locations) {
				if (players != null){
					for (Player p : players) {
						display(new NoteColor((int) offsetX), center, p);
					}
				} else {
					display(new NoteColor((int) offsetX), center, visibleRange);
				}
			}
		}else if (hasProperty(ParticleProperty.REQUIRES_DATA)) {
			if ((this == ParticleEffect.FALLING_DUST || this == ParticleEffect.BLOCK_BREAK || this == ParticleEffect.BLOCK_DUST || this == ParticleEffect.TILE_BREAK || this == ParticleEffect.TILE_DUST) && dataMat != null){
				BlockData finalData = new BlockData(dataMat, dataID);
				for (Location center : locations) {
					if (players != null){
						for (Player p : players) {
							display(finalData, offsetX.floatValue(), offsetY.floatValue(), offsetZ.floatValue(), speed.floatValue(), particleCount, center, p);
						}
					} else {
						display(finalData, offsetX.floatValue(), offsetY.floatValue(), offsetZ.floatValue(), speed.floatValue(), particleCount, center, visibleRange);
					}
				}
			} else if((this == ParticleEffect.ITEM_BREAK) && dataMat != null){
			...
			for (Location center : locations) {
				if (players != null){
					for (Player p : players) {
						display(offsetX.floatValue(), offsetY.floatValue(), offsetZ.floatValue(), speed.floatValue(), particleCount, center, p);
					}
				} else {
					display(offsetX.floatValue(), offsetY.floatValue(), offsetZ.floatValue(), speed.floatValue(), particleCount, center, visibleRange);
				}
			}
		}
	}
